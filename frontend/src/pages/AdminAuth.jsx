...
function SignupForm({ onSuccess }) {
  ...
  async function submit(e) {
    ...
    try {
      ...
      setMsg({ text: 'Admin account created. You can now sign in.', type: 'success' });
      onSuccess();
    } catch (err) {
      setMsg({ text: err.message, type: 'error' });
    } finally {
...
export default function AdminAuth({ defaultTab = 'login' }) {
  ...
  return (
    <div className="page-narrow">
      <div className="form-card">
        <p style={{ fontSize: '0.75rem', color: 'var(--muted)', marginBottom: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
          Admin portal
        </p>
        ...
        <p style={{ marginTop: '1.25rem', fontSize: '0.8rem', color: 'var(--muted)' }}>
          Looking to take courses? <Link to="/user/login" style={{ color: 'var(--fg)' }}>User sign in →</Link>
        </p>
      </div>
    </div>
  );
