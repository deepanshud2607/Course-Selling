  ...
  return (
    <article className="group flex flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
      {imageURL ? (
        ...
        <div className="grid h-48 w-full place-items-center bg-indigo-50 text-sm font-medium text-indigo-400">
          Course image
        </div>
      )}
      ...
      <div className="border-t border-slate-100 p-4">
        <Link to={`/course/${_id}`}>
          <button className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm font-semibold text-slate-700 transition hover:border-indigo-300 hover:bg-indigo-50 hover:text-indigo-700">
            View course
          </button>
        </Link>
      </div>
